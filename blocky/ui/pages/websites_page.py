...
class WebsitesPage(Gtk.Box):
    ...
    def _make_rule_row(self, rule: BlockRule) -> Gtk.ListBoxRow:
        ...
        # Type badge
        type_badge = Gtk.Label(label="WEB")
    ...
    def _on_toggle(self, switch, state, rule: BlockRule) -> bool:
        ...
        try:
            if state:
                ...
                self.window.show_toast(f"Blocked: {rule.domain}")
            else:
                bm.deactivate_rule(rule)
                self.window.show_toast(f"Paused: {rule.domain}")
            self.refresh()
    ...
    def _on_delete(self, _btn, rule: BlockRule) -> None:
        ...
        if bm:
            try:
                ...
                self.window.show_toast(f"Error: {e}")
                return
        self.window.show_toast(f"Removed: {rule.domain}")
        self.refresh()
